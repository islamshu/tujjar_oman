    ...
    public function search(Request $request)
    {
        ...
        switch ($scope) {
            case 'price_low_to_high':
                $collection = new SearchProductCollection(Product::where('published', 1)->where('name', 'like', "%{$key}%")->orWhere('name_ar', 'like', "%{$key}%")->orWhere('tags', 'like', "%{$key}%")->orderBy('unit_price', 'asc')->paginate(10));
                $collection->appends(['key' => $key, 'scope' => $scope]);
                return $this->sendResponse($collection, translate('price_low_to_high'));
            case 'price_high_to_low':
                $collection = new SearchProductCollection(Product::where('published', 1)->where('name', 'like', "%{$key}%")->orWhere('name_ar', 'like', "%{$key}%")->orWhere('tags', 'like', "%{$key}%")->orderBy('unit_price', 'desc')->paginate(10));
                $collection->appends(['key' => $key, 'scope' => $scope]);
                return $this->sendResponse($collection, translate('price_high_to_low'));
            case 'new_arrival':
                $collection = new SearchProductCollection(Product::where('published', 1)->where('name', 'like', "%{$key}%")->orWhere('name_ar', 'like', "%{$key}%")->orWhere('tags', 'like', "%{$key}%")->orderBy('created_at', 'desc')->paginate(10));
                $collection->appends(['key' => $key, 'scope' => $scope]);
                return $this->sendResponse($collection, translate('new_arrival'));
            case 'popularity':
                $collection = new SearchProductCollection(Product::where('published', 1)->where('name', 'like', "%{$key}%")->orWhere('name_ar', 'like', "%{$key}%")->orWhere('tags', 'like', "%{$key}%")->orderBy('num_of_sale', 'desc')->paginate(10));
                $collection->appends(['key' => $key, 'scope' => $scope]);
                return $this->sendResponse($collection, translate('popularity'));
            case 'top_rated':
                $collection = new SearchProductCollection(Product::where('published', 1)->where('name', 'like', "%{$key}%")->orWhere('name_ar', 'like', "%{$key}%")->orWhere('tags', 'like', "%{$key}%")->orderBy('rating', 'desc')->paginate(10));
                $collection->appends(['key' => $key, 'scope' => $scope]);

...
<div class="card">
    <form class="" action="" method="GET">
      <div class="card-header row gutters-5">
        ...
        <div class="col-lg-2">
            <div class="form-group mb-0">
                <input type="text" class=" form-control" value="{{ $customer }}" name="customer" placeholder="{{ translate('Filter by customer') }}" >
            </div>
        </div>
        <div class="col-lg-2">
            <div class="form-group mb-0">
                <input type="text" class=" form-control" value="{{ $vendor }}" name="vendor" placeholder="{{ translate('Filter by vendor') }}" >
            </div>
        </div>
        <div class="col-lg-2">
          <div class="form-group mb-0">
            ...
            <tbody>
                @foreach ($orders as $key => $order_id)
                    @php
                        ...
                        $order_de = \App\OrderDetail::where('order_id',$order_id->id)->first();
                        ...
                        <tr>
                            ...
                            <td>
                                {{ count($order->orderDetails->where('seller_id', '!=', $admin_user_id)) }}
                                    ...
                                    @php
                        $type = @App\User::find( $order_de->product->user_id )->user_type;
                        @endphp
                        @if($type == 'admin' )
                                                <td>{{@get_setting('website_name')}}</td>
                                                @else 
                                                <td>{{ @App\User::find( $order_de->product->user_id)->name}}</td>
                        @endif

        ...
        <div class="card-body">
          <form action="{{ route('sellers.update', $seller->id) }}" method="POST">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="phone" placeholder="{{translate('phone')}}" id="phone" name="phone" class="form-control" value="{{$seller->user->phone}}" required>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="password">{{translate('Seller Packages')}}</label>
                    <div class="col-sm-9">
                      <select name ="pakege" class="form-control" required>
                          @foreach($packeges as $pakege)
                          <option value="{{$pakege->id}}"@if($pakege->id == $seller->vendor_pakege_id) selected @endif>{{$pakege->get_title()}}</option>
                          @endforeach

	...
	<div style="padding: 1.5rem;">
		<table class="padding text-left small border-bottom">
			...
			<tr class="gry-color" style="background: #eceff4;">
				...
				<th width="10%">{{ translate('Product Name') }}</th>
				<th width="25%">{{ translate('Attribute') }}</th>
			...
			@foreach ($order->orderDetails as $key => $orderDetail)
				@if ($orderDetail->product != null)
					<tr class="">
						<td width="15%"class="text-right currency">{{ single_price($orderDetail->price+$orderDetail->tax) }}</td>
						<td width="10%" class="gry-color currency">{{ single_price($orderDetail->tax/$orderDetail->quantity) }}</td>
						<td  width="15%" class="gry-color currency">{{ single_price($orderDetail->price/$orderDetail->quantity) }}</td>
						<td width="10%" class="gry-color">{{ $orderDetail->quantity }}</td>
						<td width="15%">
						...
						<td  width="10%">{{ $orderDetail->product->getTranslation('name') }}</td>
						@if ($orderDetail->variation != null)
							<td   class="text-center">@if($orderDetail->atts()['color'] != null) اللون : {{ $orderDetail->atts()['color'] }} @endif <br>
								@if(@$orderDetail->atts()['size'] != null) القياس : {{ @$orderDetail->atts()['size'] }} @endif <br>
								@if(@$orderDetail->atts()['fabric'] != null) الخواص : {{ @$orderDetail->atts()['fabric'] }} @endif <br>

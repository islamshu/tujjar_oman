                ...
                <li class="list-group-item">
                    <a class="text-reset" href="{{ route('product', $product->slug) }}">
                        <div class="d-flex search-product align-items-center">
                            <div class="mr-3">
                                <img class="size-40px img-fit rounded"  onerror="this.onerror=null;this.src='{{ uploaded_asset($error_product) }}';" src="{{ uploaded_asset($product->thumbnail_img) }}">
                            </div>
                ...
                @foreach ($shops as $key => $shop)
                    <li class="list-group-item">
                        <a class="text-reset" href="{{ route('shop.visit', $shop->slug) }}">
                            <div class="d-flex search-product align-items-center">
                                <div class="mr-3">
                                    <img class="size-40px img-fit rounded" onerror="this.onerror=null;this.src='{{ uploaded_asset($error_product) }}';" src="{{ uploaded_asset($shop->logo) }}">
                                </div>

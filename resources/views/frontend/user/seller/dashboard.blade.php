                ...
                <div class="aiz-titlebar mt-2 mb-4">
                  ...
                  <div class="row">
                      ...
                      <div class="col-md-4">
                          ...
                          @endif
                          @php
                          $paid = Auth::user()->seller->paid;
                          $vendor_pakege_id = Auth::user()->seller->vendor_pakege_id;
                          @endphp
                          @if($paid !=1)
                          <div class="bg-white mt-4 p-4 text-center">
                              <div class="h5 fw-600">{{ translate('Subscription')}}</div>
                              <p>{{ App\Vendorpackege::findOrfail($vendor_pakege_id)->title}}&nbsp;({{single_price(App\Vendorpackege::findOrfail($vendor_pakege_id)->price)}})</p>

                              <a  onclick="show_price_modal(2)" class="btn btn-soft-primary">{{ translate('pay  to subscribe')}}</a>
                          </div>
                          @endif
                          <div class="bg-white mt-4 p-4 text-center">
                              ...
                              @endphp
                              <a href="{{ route('shops.index',['lang'=>$lang]) }}" class="btn btn-soft-primary">{{ translate('Go to setting')}}</a>
                          </div>
...
@section('modal')
    ...
    <div class="modal fade" id="price_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ translate('Subscription') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
                </div>
                <form class="" id="package_payment_form" action="{{ route('subscription') }}" method="post">
                    @csrf
                    <input type="hidden" name="vendor_pakege_id" value="{{Auth::user()->seller->vendor_pakege_id}}">
                            ...
                            <div class="col-md-10">
                                <div class="mb-3">
                                    <select class="form-control selectpicker" data-live-search="true" name="payment_option">
                                        ...
                                        @endif
                                         @if (\App\BusinessSetting::where('type', 'thawani')->first()->value == 1)
                                            <option value="thawani">{{ translate('Thawani')}}</option>
                                        @endif
